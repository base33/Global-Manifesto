...
angular.module("umbraco")
    .filter("wbContainerName", function () {
...
angular.module("umbraco")
    ...
    function ($scope, $http: ng.IHttpService, $element: JQuery, appState: any, contentResource: any, editorState: any, eventsService: any, assetsService, dialogService, notificationsService, $compile: ng.ICompiledExpression) {
        ...
        $scope.getSortableOptions = function (blockList : Array<WebBlocks.LayoutBuilder.Block>) {
            return {
                ...
                modelData: blockList,
                over: function (e, ui) {
                    $scope.currentSortableHoveredContainer = <WebBlocks.LayoutBuilder.Container>$scope.$eval($(e.target).attr("wb-container-model"));
                },
                update: function (e, ui) {
                    //if sender is not null, this means the container which had the block dropped on it is being updated
                    if (ui.sender != null) {
                        ////get the current container
                        //var container = <WebBlocks.LayoutBuilder.Container>$scope.$eval($(e.target).attr("wb-container-model"));
                        //var block = <WebBlocks.LayoutBuilder.Block>$scope.currentSortableDraggedBlock;
                        ...
                        var newContainer = <WebBlocks.LayoutBuilder.Container>$scope.currentSortableHoveredContainer;
            ...
            if (draggableBlockModel.LoadContent == true && draggableBlockModel.Block instanceof WebBlocks.LayoutBuilder.NodeBlock) {
                ...
                loadBlockContent(block, function () {
                    $scope.$apply(function () {
                        ...
                        if (!WebBlocks.API.WebBlocksAPIClent.ValidateRenderedBlock(<WebBlocks.LayoutBuilder.NodeBlock>block)) {
                        ...
                        contentResource.getById(block.Id).then(function (content) {
                            ...
                            (<WebBlocks.LayoutBuilder.NodeBlock>block).ContentTypeAlias = content.contentTypeAlias;
        ...
        function loadBlockContent(block: WebBlocks.LayoutBuilder.Block, callback: () => void) {
            var url = "/umbraco/surface/BlockRenderSurface/RenderBlock?wbPreview=true&pageId=" + editorState.current.id
                + "&blockId=" + block.Id;
            $.ajax({
                type: 'GET',
                url: url,
        ...
        $scope.editBlock = function (blockElement, block: WebBlocks.LayoutBuilder.Block, container: WebBlocks.LayoutBuilder.Block, applyScope: boolean = true) {
            if (block instanceof WebBlocks.LayoutBuilder.NodeBlock) {
                ...
                if (block.Content != block.ViewModel.Html) return;
        ...
        $scope.handleEditBlockDialog = function (event: WebBlocks.UI.Dialogs.ContextMenuResponse) {
            ...
            var container = <WebBlocks.LayoutBuilder.Container>event.EventData.container;
            switch (event.Event) {
                case 'Edit':
        ...
        function loadLayoutBuilder(callback: () => void): WebBlocks.LayoutBuilder.LayoutBuilder {
            ...
            previewProvider.GetPreview(editorState.current.id, $http,
                function (preview) {
                    ...
                    angular.forEach($scope.model.value.Containers,(savedContainer: WebBlocks.LayoutBuilder.Container, savedContainerName: string) => {
                        ...
                        if (!found) {
                            ...
                            for (var i = 0; i < savedContainer.Blocks.length; i++) {
                                layoutBuilderModel.BlockStorage.push(new WebBlocks.LayoutBuilder.BlockStorageBlock(savedContainer.Blocks[i], "Missing container",
                                    new WebBlocks.LayoutBuilder.BlockHistory(savedContainer.Name)));
                            }
                        }
                    });
