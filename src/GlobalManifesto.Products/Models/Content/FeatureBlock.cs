
using System;
using System.Collections.Generic;
using Umbraco.Core.Models;
using Umbraco.Web;
using System.Web;
using System.Linq;
using System.ComponentModel.DataAnnotations;
using ConcreteContentTypes.Core.Models;
using ConcreteContentTypes.Core.Interfaces;
using Newtonsoft.Json;

using System;
using RJP.MultiUrlPicker.Models;
using System.Web;
using GlobalManifesto.Products.Models.Media;
using Umbraco.Examine.Linq.Attributes;
using ConcreteContentTypes.Core.Extensions;


namespace GlobalManifesto.Products.Models.Content
{
	 [NodeTypeAlias("FeatureBlock")]
 	public partial class FeatureBlock : BlockBase
	{
		public override string ContentTypeAlias { get { return "FeatureBlock"; } }

				
		
				
		
		[Field("title")]
		public string Title { get; set; } 		
		
		private IPublishedContent _image = null;
		public IPublishedContent Image
		{
			get 
			{
				if (_image == null)
				{
					int? contentId = Content.GetPropertyValue<int?>("image", this.GetPropertiesRecursively);

					if (contentId.HasValue)
					{
					
						_image = UmbracoContext.Current.MediaCache.GetById(contentId.Value);
				
						
					}	
				}
				return _image;
			}
		} 		
		
				
		
		[Field("link")]
		public MultiUrls Link { get; set; } 		
		
				
		
		[Field("content")]
		public IHtmlString content { get; set; } 
		
		private IEnumerable<IPublishedContent> _children = null;
		[JsonIgnore]
		public IEnumerable<IPublishedContent> Children
		{
			get
			{
				if (_children == null && this.Content != null)
					_children = this.Content.Children;

				return _children;
			}
		}

		public FeatureBlock()
			: base()
		{
		}

		public FeatureBlock(string name, IConcreteModel parent)
			: this(name, parent.Id)
		{
		}

		public FeatureBlock(string name, int parentId)
			: base()
		{
			this.Name = name;
			this.ParentId = parentId;
		}

		public FeatureBlock(int contentId, bool getPropertiesRecursively = false)
			: base(contentId, getPropertiesRecursively)
		{
		}

		public FeatureBlock(IPublishedContent content, bool getPropertiesRecursively = false)
			: base(content, getPropertiesRecursively)
		{
		}

		public override void Init(IPublishedContent content)
		{
			base.Init(content);
						
			this.Title = Content.GetPropertyValue<string>("title", this.GetPropertiesRecursively);
						
			this.Link = Content.GetPropertyValue<MultiUrls>("link", this.GetPropertiesRecursively);
						
			this.content = Content.GetPropertyValue<IHtmlString>("content", this.GetPropertiesRecursively);
			
		}

	}
}

